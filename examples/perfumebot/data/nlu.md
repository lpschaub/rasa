...
## intent:purchase
tenir informé je veux bien le prendre avec d autre produit mais au prix indiqué de36.99
...
bonjour je voudrais savoir si l'eau de toilette en [200ml](size) pour un [homme](target) de chez _Name1_ a un emballage [cadeau](present) et combien de temps de livraison a [domicile](home) avec les frais de port
## intent:product+AskDetails
merci mais il est quand même dans une boite cartonnée
## intent:customer_service
_Name2_ : je viens d'envoyer à l'instant sur le mail indiqué les documents reçus avec ma commande.
## intent:CustomerComplaint+goodbye
_Name2_ : Ok merci je vais donc enregistrer votre conversation car toutes les commandes passées [chez vous](store) le [prélèvement](billing) est toujours différents bonne journée
...
_Name2_ : bonjour une question sur le [fond de teint make up for ever fluide tenseur](article)   le coloris [sable](color) est il bien [clair](color) merci
...
_Name4_ : alors votre offre est pas correcte car elle devrait le spécifier6 _Name4_ : étant donné que votre [site](website) comporte pratiquement que des articles avec [remise](discount)
## intent:CancelOrder
_Name2_ : bonjour je viens de passer à l'instant ma commande qui a pour numéro [00](order) _Name2_ : seulement je viens de me rendre [compte](account) que je me suis trompée etc. y a t il possibilité d'annuler celle ci etc ?
...
ok d'accord je vais attendre10 je vous remercie
...
## intent:deliveryTime
super.6 pensez vous que je serai [livrée](delivery) la [semaine](date) prochaine ? je vis en [Martinique](place) et viens en [métropole](place) ce [samedi](date) pour une semaine. je demande la livraison [chez mon fils](home) à [_City1_](city)
...
_Name2_ : pourquoi je n'ai pas eu de [message](channel) pour me prévenir des références du 2eme colis avec un numéro de [suivi](follow-up)
...
_Name2_ : ok je regarde10 _Name2_ : dommage que les + vieux messages des indésirables soient du [23 juillet](date) mais dans la liste des indésirables il n'y a pas aucun de votre [site](website) etc ce qu'il serait bien est que vous me l'envoyez de nouveau ce mail etc merci
## intent:oui+goodbye
_Name2_ : ok j'ai enregistre le [suivi](follow-up) etc etc mais je [reste](missing) septique pour le 2eme mail étant donne que le 1er mail y a pas eu de problème pour le réception et le suivi14 _Name2_ : bonne journée
## intent:thanks+goodbye+autre
_Name3_ : Merci j'attend le deuxième colis11 _Name3_ : bonne journée
## intent:Refund
_Name2_ : si je vous [renvoi](resend) le colis je serais [remboursé](refund) ?
- _Name1_ : pensez à [déduire](discount) les 10€ oubliés de la commande initiale
## intent:deliveryCost+Discount
_Name2_ : bonjour je souhaite acheter 4 flacons de [parfum](article) mais je trouve le [tarif de livraison](fdp) très cher etc ! feriez vous vous un [geste commercial](discount) ? Merci
...
_Name3_ : bonjour j'ai déjà eu une conversation avec vous et je viens d'être " " coupée " " avec _Name1_ ;2 _Name3_ : pouvez vous relancer [Chronopost](transporter) puisque le colis est arrivé chez eux le [1er octobre](date) et nous sommes le [10](order) octobre.
...
_Name2_ : _Name4_ veut dire _Name3_ dune [semaine](date)
## intent:oui+deliveryPlace
_Name2_ : Ok merci je préfère une livraison expresse a domicile5 _Name2_ : je valide mon achat
## intent:deliveryPlace
in _Name3_ please ?2 I have an order : _Name4_ NR : [0201 7041 4639 69773](order) I do not know Where the order has been [delivered](delivery) ?4 are you taire ?
- _Name1_ : je suis en [France](place) normalement.
- alirs faites la livraison en point relais14 merci de le confirmer
## intent:delivery
_Name2_ : _Email1_2 _Name2_ : bonjour un article de ma commande a été [retourné](resend) que doit je faire maintenant merci
...
_Name2_ : vous voulez dire 2e envoi car depuis le 26 Mars il y a UN gros SOUCI8
## intent:deliveryNews+AskDetails+CustomerComplaint
_Name2_ : vous êtes Sure que je serai livré [cette année](date) ! cela fait 3 fois que l'on me donne de nouveaux délais
## intent:saluer
_Name2_ : bonjour ?
- _Name2_ : bonjour
- _Name1_ : bonsoir
## intent:AskDetails+StoreLocation+deliveryTime
_Name3_ : non quand je consulte le [lien](follow-up) sur [chrono post](transporter) il est prêt chez l'expéditeur. vous ? depuis le [31/12](date) etc vous n'êtes pas en [France](place) ?6 _Name3_ : une petite idée du délai ?
## intent:deliveryTime+AskDetails
tétard : donc depuis le [31/12](date) ça suppose que je le reçoive entre le [aujourd'hui](date) et [samedi](date) ? si je n'ai rien [samedi](date) qu'est ce que je fais ?
## intent:saluer+deliveryNews
_Name2_ : bonjour2 vernat : je voudrais savoir où en est ma commande
## intent:ChangeData
_Name1_ : peut on changer le [mot de passe](pwd) que j'ai marqué
- par contre l'adresse n'est plus la même je n'habite plus à [_City1_](city)
## intent:deliveryTime+deliveryPlace
_Name2_ : est ce que vous pouvez me dire à peut prêt quand il va être livré et si [Chronopost](transporter) le met dans la [boîte aux lettre](home) en cas d'ansence5 _Name2_ : car je pars [vendredi](date) et je reviens dimanche6 _Name2_ : [00](order) _Name2_ : vous pouvez me dire ?8 _Name2_ : [rassurez](mood) moi vous trouvez bien ma commande ?
## intent:oui+CustomerComplaint
_Name1_ : absolument9 _Name1_ : mais je trouve que ce n'est pas sérieux de votre part
## intent:autre+goodbye
_Name1_ : _Name3_ je le fais tout de suite7 _Name1_ : Merci
## intent:Discount+non
_Name2_ : c'est ça j'ai fait mai Ca passe ps la [réduction](discount)
## intent:deliveryNews+deliveryPlace
alors que faire ?10 merci de faire le revoir du colis à mon domicile11 202 rue du château des rentiers12 _ZipCode1_
## intent:non+goodbye
_Name2_ : pas dans l'immédiat bonne journée
## intent:Discount+goodbye
oui j'ai eu mais il faut minimum 60 euros de commande6 pas grave merci quand même bonne journée
## intent:PaymentTool+PaymentRefused
_Name2_ : je rentre mon numéro de [carte](tool) et je valide ça me marque refuse
## intent:CustomerComplaint+AccountCreation
qui est la personne qui a créer mon [compte](account) hier un arnaqueur16 avec d'autres [sites](website) aucun problèmes
## intent:AccountIssue+CustomerComplaint
l'adresse ne passe pas motif votre numéro de [domicile](home) n'est pas valide utilisez seulement [10](order) chiffres sans espace sans point18 je pense que votre société fait téléphoner les gens pour le rapport du tel
...
_Name2_ : je n'ai reçu aucun [message](channel) pour me prévenir etc etc5 _Name2_ : merci du renseignement et bonne journée à vous
