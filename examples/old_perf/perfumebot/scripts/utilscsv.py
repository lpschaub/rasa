...
def allinonefile(filedir, outfile):
    for file in filedir:
        outfile.write(open(file, encoding='utf-8').read() + '\n')
    outfile.close()
...
if __name__ == '__main__':
    ...
    print(keys)
    dir = "../input/norm/"
    fiftyfold = "../input/50fold/"
    os.mkdir(fiftyfold)
    for i in range(50):
        for conv in glob.glob(f'{dir}*'):
            if conv.split('/')[-1] not in keys and not os.path.isfile(fiftyfold+conv.split('/')[-1]) :
                print(conv)
                os.system(f"cp {conv} {fiftyfold}")
                break
"""
    ...
    # for file in  glob.glob(DATA+"anon/*"):
    #
    # f = open(file).read()
    #
    # print(file.split('/')[-1])
    # out = open(DATA+'clean/'+file.split('/')[-1],'w',encoding='utf-8')
    #
    # out.write(ano2(f))
    # ls
